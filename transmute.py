...
def main():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "hdtlLcw:p:P:i:I:nsaA", ["help", "list="]
        )
...
def GoTransmuteCaps(origLine):
    ...
    if True == CONF_capitalize:
        TransmuteCaps(
            origLine, GoTransmuteCommonLeet
        )  # change to common leet transmute
    else:
...
def getNextSubstitute(curVal, valMatrix, curIndex=0):
    ...
    if next >= len(valMatrix[curIndex]):
        ...
        return getNextSubstitute(
            curVal[:curIndex] + [valMatrix[curIndex][0]] + curVal[curIndex + 1 :],
            valMatrix,
            curIndex + 1,
        )
    else:
